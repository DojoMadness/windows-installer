...
let createWindowsInstaller = exports.createWindowsInstaller = (() => {
  var _ref = (0, _bluebird.coroutine)(function* (options) {
    ...
    const vendorUpdate = _path2.default.join(vendorPath, 'Squirrel.exe');
    ...
    cmd = _path2.default.join(vendorPath, 'Squirrel.com');
    args = ['--releasify', nupkgPath, '--releaseDir', outputDirectory, '--loadingGif', loadingGif];

    if (useMono) {
      args.unshift(_path2.default.join(vendorPath, 'Squirrel-Mono.exe'));
      cmd = monoExe;
    }
    ...
    if (options.frameworkVersion) {
      args.push(`--framework-version=${options.frameworkVersion}`);
    }
